...
from object_detection.model.utils.bbox_tools import loc2bbox
from torchvision.ops import nms
from torch import nn
from object_detection.dataset.dataset import preprocess
...
class FasterRCNN(nn.Module):
    ...
    def forward(self, x, scale=1.):
        ...
        rpn_locs, rpn_scores, rois, roi_indices, anchor = self.rpn(feature_map, img_size, scale)
        roi_cls_locs, roi_scores = self.head(feature_map, rois, roi_indices)
    ...
    def _suppress(self, raw_cls_bbox, raw_prob):
        ...
        label = list()
        score = list()
        raw_cls_bbox_reshape = raw_cls_bbox.reshape((-1, self.n_class, 4))
        # skip cls_id = 0 because it is the background class
        for l in range(1, self.n_class):
            cls_bbox_l = raw_cls_bbox_reshape[:, l, :]
            prob_l = raw_prob[:, l]
