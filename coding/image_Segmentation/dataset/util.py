...
if __name__ == '__main__':
    ...
    img_dir = os.path.join(base, 'glas_dataset')

    data = os.path.join(base, 'img_npy', 'testA_1.npy')
    label = os.path.join(base, 'img_npy', 'testA_1_anno.npy')

    d = np.load(data)
    l = np.load(label)
    import image_Segmentation.metric as m
    print(m.Dice(l,l))

    #
    # plt.imshow(np.where(l > 0, 1., 0.))
    # plt.show()
    #
    # plt.imshow(d / 255.)
    # plt.show()
